...
function RequestAdd(props) {
    ...
    const AddRequest = async (data) => {
        try {
            ...
            const response = await axios.put(`/requesters/${props.obj._id}`, data); //Without Toke Request

            if (response)
                swal("Done!", `${data.name}'s Request on ${data.when} has been updated!`, "success");
            else
                        ...
                        <div className="row">
                            <div className="form-group col-md-2"> { /*Is Processed */}
                                ...
                                <select
                                    ...
                                    defaultValue={props.obj.isProceed}
                                    >
